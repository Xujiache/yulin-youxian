...
微信支付始终调用真实 API v3 接口；必须配置商户号、商户私钥、商户证书序列号、API v3 密钥、微信支付公钥（推荐，含公钥 ID）或平台证书，以及公网 HTTPS 回调地址。
生产部署设置为 `false` 后，微信支付调用真实 API v3 接口，必须配置商户号、商户私钥、商户证书序号、API v3 密钥、微信支付公钥（推荐，含公钥 ID）或平台证书，以及公网 HTTPS 回调地址。
