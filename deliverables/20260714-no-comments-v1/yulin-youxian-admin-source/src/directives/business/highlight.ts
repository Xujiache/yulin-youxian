...
function isBlockProcessed(block: HTMLElement): boolean {
    return (block.hasAttribute('data-highlighted') ||
...
function retryProcessing(el: HTMLElement, maxRetries: number = 3, delay: number = 200) {
    ...
    const tryProcess = () => {
        ...
        const remainingBlocks = Array.from(el.querySelectorAll<HTMLElement>('pre code')).filter((block) => !isBlockProcessed(block));
