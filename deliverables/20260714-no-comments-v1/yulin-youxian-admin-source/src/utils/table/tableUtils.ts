...
function extractPagination(obj: Record<string, unknown>, data?: Record<string, unknown>): Pick<ApiResponse<unknown>, 'current' | 'size'> | undefined {
...
export const defaultResponseAdapter = <T>(response: unknown): ApiResponse<T> => {
    ...
    if (typeof response !== 'object') {
        console.warn('[tableUtils] 无法识别的响应格式，支持的格式包括: 数组、包含' +
            recordFields.join('/') +
            '字段的对象、嵌套data对象。当前格式:', response);
        return { records: [], total: 0 };
    }
...
export const updatePaginationFromResponse = <T>(pagination: Api.Common.PaginationParams, response: ApiResponse<T>): void => {
...
export const createSmartDebounce = <T extends (...args: any[]) => Promise<any>>(fn: T, delay: number): T & {
    cancel: () => void;
    flush: () => Promise<any>;
} => {
