...
class FireworkSystem {
    ...
    private renderShape(particle: Firework): void {
        ...
        switch (particle.shape) {
            case 'rectangle':
                ctx.value.fillRect(-SIZES.RECTANGLE.WIDTH / 2, -SIZES.RECTANGLE.HEIGHT / 2, SIZES.RECTANGLE.WIDTH, SIZES.RECTANGLE.HEIGHT);
                break;
            case 'square':
    ...
    private renderImage(particle: Firework): void {
        if (!ctx.value || !particle.imageUrl)
            return;
...
const handleKeyPress = (event: KeyboardEvent): void => {
    const isFireworkShortcut = (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'p') ||
