...
class VersionManager {
    ...
    private findLegacyStorage(): {
        oldSysKey: string | null;
        oldVersionKeys: string[];
    } {
        const storageKeys = Object.keys(localStorage);
        const currentVersionPrefix = StorageConfig.generateStorageKey('').slice(0, -1);
        const oldSysKey = storageKeys.find((key) => StorageConfig.isVersionedKey(key) && key !== currentVersionPrefix && !key.includes('-')) || null;
        const oldVersionKeys = storageKeys.filter((key) => StorageConfig.isVersionedKey(key) &&
    ...
    private buildUpgradeMessage(requireReLogin: boolean): string {
        ...
        if (requireReLogin) {
            messageParts.push(`<p style="color: var(--theme-color); padding-top: 5px;">升级完成，请重新登录后继续使用。</p>`);
        }
    ...
    private async executeUpgrade(storedVersion: string, legacyStorage: ReturnType<typeof this.findLegacyStorage>): Promise<void> {
        try {
