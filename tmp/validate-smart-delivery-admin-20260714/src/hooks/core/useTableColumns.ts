...
const SPECIAL_COLUMNS: Record<string, {
    prop: string;
    label: string;
}> = {
...
export const getColumnChecks = <T>(columns: ColumnOption<T>[]) => columns.map((col) => {
...
export function useTableColumns<T = any>(columnsFactory: () => ColumnOption<T>[]): {
    ...
    watch(dynamicColumns, (newCols) => {
        const visibilityMap = new Map(columnChecks.value.map((c) => [getColumnKey(c), getColumnVisibility(c)]));
    ...
    return {
        columns,
        columnChecks,
        addColumn: (column: ColumnOption<T> | ColumnOption<T>[], index?: number) => setDynamicColumns((cols) => {
            const next = [...cols];
            const columnsToAdd = Array.isArray(column) ? column : [column];
            const insertIndex = typeof index === 'number' && index >= 0 && index <= next.length ? index : next.length;
            next.splice(insertIndex, 0, ...columnsToAdd);
            return next;
        }),
        removeColumn: (prop: string | string[]) => setDynamicColumns((cols) => {
        ...
        batchUpdateColumns: (updates) => setDynamicColumns((cols) => {
        ...
        reorderColumns: (fromIndex: number, toIndex: number) => setDynamicColumns((cols) => {
            if (fromIndex < 0 ||
