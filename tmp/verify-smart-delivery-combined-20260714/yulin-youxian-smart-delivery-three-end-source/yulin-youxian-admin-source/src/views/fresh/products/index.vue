...
import { createProduct, deleteProduct, getCategories, getProducts, updateProduct, updateProductStatus, updateProductStock, uploadProductImage, type Category, type Product } from '@/api/admin';
...
const categoryName = (categoryId: number | null) => categories.value.find((item) => item.id === categoryId)?.name || '未分类';
...
const removeProduct = async (row: Product) => {
    if (!row.id)
        return;
    try {
        await ElMessageBox.confirm(`确认删除商品「${row.name}」？商品将从商城和购物车移除，历史订单记录仍会完整保留。`, '删除商品', {
            type: 'warning',
