...
Page({
  data: { themeStyle: '', showBack: false, state: 'loading', errorText: '', items: [], page: 1, pageSize: 20, total: 0, loadingMore: false },
  onLoad() {
    this.loadEpoch = 0;
    this.resetLoading = false;
    this.setData({ themeStyle: cssVariables(), showBack: getCurrentPages().length > 1 });
    this.offEvent = events.on('socket-event', () => this.load(true));
  ...
  load(reset) {
    ...
    if (reset) { this.resetLoading = true; this.setData({ state: 'loading', errorText: '', loadingMore: false }); }
    else this.setData({ loadingMore: true });
    return api.listFeedbacks({ page, pageSize: this.data.pageSize })
      .then((result) => {
      ...
      .catch((error) => { if (epoch === this.loadEpoch) this.setData({ state: reset ? 'error' : this.data.state, errorText: userMessage(error) }); })
      .finally(() => {
        if (epoch !== this.loadEpoch) return;
  ...
  open(event) { navigation.navigate('detail', { id: event.currentTarget.dataset.id }); },
  create() { navigation.navigate('submit'); },
  knowledge() { navigation.navigate('knowledge'); }
});
